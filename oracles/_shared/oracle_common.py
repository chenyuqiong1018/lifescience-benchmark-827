...
REQUIRED_OUTPUTS = {
    ...
    "ls07-combination-treatment-mechanism": ["pathway_enrichment.csv", "mechanism_call.json", "analysis.py", "report.md"],
...
def run(task_id: str) -> int:
    ...
    checker = _load_scientific_checker(oracle_dir)
    accepted = bool(checker and getattr(checker, "ACCEPTED", False))
    if not accepted:
        result = {
            "task_id": task_id,
        ...
        scientific = checker.check(workspace)  # checker reads frozen inputs/outputs; never imports submission code
